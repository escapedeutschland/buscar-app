...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #app { display: flex; flex-direction: column; height: 100vh; max-width: 430px; margin: 0 auto; background: var(--bg); position: relative; overflow: auto; }
    ...
    /* ── Auth Screen ─────────────────────────── */
    .auth-screen { background: var(--card); }
    .auth-header {
    ...
    .auth-logo-icon {
      width: 64px; height: 64px; background: rgba(255,255,255,0.22);
    ...
    .auth-tab {
      ...
      font-size: 14px; font-weight: 500; color: var(--text-2); cursor: pointer; transition: all 0.2s;
    }
    ...
    .field-input {
      ...
      color: var(--text-1); background: var(--card); outline: none; transition: border-color 0.2s;
    }
    .field-input:focus { border-color: var(--yellow); }
    .auth-btn {
      width: 100%; padding: 16px; background: var(--yellow); color: white;
    ...
    .auth-btn:active { transform: scale(0.98); background: var(--yellow-dark); }
    ...
    .search-box {
      ...
      padding: 0 16px; gap: 10px; height: 50px;
    }
    ...
    .profil-row-arrow svg { width: 16px; height: 16px; stroke: var(--text-3); }
...
<div id="app">
  ...
  <!-- ── AUTH SCREEN ────────────────────────── -->
  <div class="screen auth-screen active" id="screenAuth">
    ...
    <div class="auth-body">
      ...
      <form class="auth-form" id="formLogin" onsubmit="handleLogin(event)">
        <div>
          ...
          <input class="field-input" type="email" id="loginEmail" placeholder="deine@email.de" required>
        </div>
        <div>
          <label class="field-label">Passwort</label>
          <input class="field-input" type="password" id="loginPassword" placeholder="••••••••" required>
        </div>
      ...
      <form class="auth-form hidden" id="formRegister" onsubmit="handleRegister(event)">
        <div>
          ...
          <input class="field-input" type="text" id="regName" placeholder="Dein Name" required>
        </div>
        <div>
          <label class="field-label">E-Mail</label>
          <input class="field-input" type="email" id="regEmail" placeholder="deine@email.de" required>
        </div>
        <div>
          <label class="field-label">Passwort</label>
          <input class="field-input" type="password" id="regPassword" placeholder="Min. 6 Zeichen" required minlength="6">
        </div>
  ...
  <div class="screen" id="screenHome">
    ...
    <nav class="bottom-nav">
      <div class="nav-item active" onclick="showScreen('screenHome')">
      ...
      <div class="nav-item">
        <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg>
        <span>Karte</span>
      </div>
      <div class="nav-item">
        <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
        <span>Eintrag</span>
      </div>
      <div class="nav-item" onclick="showScreen('screenProfil')">
    ...
    <div class="profil-body">
      <div class="profil-section">
        <div class="profil-row" onclick="showScreen('screenHome')">
          ...
          <div class="profil-row-arrow"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg></div>
        </div>
      </div>
      <button class="logout-btn" onclick="handleLogout()">Ausloggen</button>
    </div>
    <nav class="bottom-nav">
      <div class="nav-item" onclick="showScreen('screenHome')">
        <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        <span>Home</span>
      </div>
      ...
      <div class="nav-item active" onclick="showScreen('screenProfil')">
        <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        <span>Profil</span>
      </div>
    </nav>
...
<script>
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      showError(msgs[err.code] || 'Fehler beim Einloggen. Bitte versuche es erneut.');
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      await db.collection('users').doc(cred.user.uid).set({
        name, email, created_at: new Date(), verified: false
      });
    } catch (err) {
      const msgs = {
  ...
  function renderListings() {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(l => (l.name||'').toLowerCase().includes(q) || (l.description||'').toLowerCase().includes(q) || (l.city||'').toLowerCase().includes(q));
    }
    ...
    if (!filtered.length) {
      container.innerHTML = '<div class="empty-state"><div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div><div class="empty-title">Nichts gefunden</div><div class="empty-sub">Versuch einen anderen Suchbegriff</div></div>';
      return;
    }
      ...
      <div class="listing-card">
        ...
        <div class="listing-body">
          <div class="listing-top">
            ...
            ${l.verified ? '<span class="badge-geprueft">✓ Geprüft</span>' : ''}
          </div>
          ${l.city ? `<div class="listing-city"><svg viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>${l.city}</div>` : ''}
          <div class="listing-desc">${l.description||''}</div>
          ${l.phone ? `<div class="listing-phone"><svg viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6 6l.9-.9a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.73 16.92z"/></svg>${l.phone}</div>` : ''}
        </div>
