const ADMIN_EMAIL = 'maximechristalle@gmail.com';
  ...
  // am selben Ort -> Liste (sonst würden die Pins sich gegenseitig verdecken).
  function buildEvGroupPopup(list) {
    if (list.length === 1) {
      var ev = list[0];
      var em1 = EVENT_TYPE_EMOJIS[ev.type] || '📅';
  ...
  async function renderEventsOnMap() {
    ...
    // Events nach Koordinate gruppieren – sonst liegen mehrere Events am selben
    // Veranstaltungsort exakt übereinander und verdecken sich gegenseitig.
    var groups = {};
    ...
    Object.keys(groups).forEach(function(key) {
      ...
      // Circle marker (mit Zähler-Badge, wenn mehrere Events an diesem Ort)
      var el = document.createElement('div');
      el.style.cssText = 'position:relative;width:38px;height:38px;border-radius:50%;background:'+color+';border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:18px;';
      el.textContent = emoji;
      if (many) {
        var badge = document.createElement('div');
        badge.textContent = g.list.length;
        badge.style.cssText = 'position:absolute;top:-5px;right:-5px;min-width:19px;height:19px;padding:0 4px;box-sizing:border-box;border-radius:10px;background:#EF4444;color:#fff;font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;border:2px solid #fff;';
